...
(defn- army-coastal-phase
  [suffix]
  (keyword "process-computer"
           (str "army-coastal-" suffix)))
...
(defn process-move-to-coast-for-transport
  ...
  (let [unit (get-in (sa/read-state :computer-map) (conj pos :contents))
    ...
    (cond
      ...
      (let [result (profiling/time-phase
                    (army-coastal-phase "transport-near-target-fill")
      ...
      (or (profiling/time-phase
           (army-coastal-phase "transport-cheap-step")
           #(step-toward-target-cheap pos target country-id))
          (profiling/time-phase
           (army-coastal-phase "transport-local-step")
           #(movement/local-step-toward-objective pos target country-id))
          (profiling/time-phase
           (army-coastal-phase "transport-move-to-target")
           #(movement/move-toward-objective pos target country-id))
          (profiling/time-phase
           (army-coastal-phase "transport-no-progress")
           (constantly nil))))))
