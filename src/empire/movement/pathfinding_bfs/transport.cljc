...
(defn- find-nearest-unload-uncached
  ...
  (let [timer (profile/begin :pathfinding/find-nearest-unload-uncached)]
    (let [game-map (current-world)
          result (loop [queue (conj clojure.lang.PersistentQueue/EMPTY start)
                        visited #{start}]
                   (when (seq queue)
...
(defn bfs-to-land-ho-target
  ...
  (let [timer (profile/begin :pathfinding/bfs-to-land-ho-target)
        passable-sea? (fn [pos]
                        (let [cell (get-in computer-map pos)]
                          (and cell (= :sea (:type cell)))))
        result (if (adjacent-to-target? start target-city)
                 []
