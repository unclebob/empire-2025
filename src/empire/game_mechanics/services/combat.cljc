  ...
  (let [city-cell (get-in world city-coords)]
    (if (< (rand) 0.5)
      (let [captured-computer-city? (= :computer (:city-status city-cell))
        ...
        {:world new-world
         :messages (merge {:error-message ""
                           :error-until 0}
                          (turn-message-map "City conquered." 2000))
      ...
      {:world world
       :messages (merge {:error-message ""
                         :error-until 0}
                        (turn-message-map (:conquest-failed config/messages) 2000))
       :combatant true})))
