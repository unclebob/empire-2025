...
(defn- map-display-label
  [map-to-display]
  ({:player-map nil
    :computer-map "Comp Map"
    :actual-map "Actual Map"} map-to-display))
...
(defn- order-context-text
  ...
  (when coords
    (case order-type
      :flight (str "Orders Flight " (format-coords coords))
      :march (str "Orders March " (format-coords coords))
      :waypoint (str "Orders Waypoint " (format-coords coords))
      nil)))
...
(defn- cell-order-context
  ...
  (let [city-marching-orders (:marching-orders cell)
    ...
    (cond
      (= city-marching-orders :lookaround) "Orders Lookaround"
