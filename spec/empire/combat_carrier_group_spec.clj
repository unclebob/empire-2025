(ns empire.application.combat-carrier-group-spec
...
(describe "clear-carrier-group-on-death"
  ...
  (context "dead battleship"
    (it "clears group-battleship-id on the paired carrier"
      ...
      (should= 7 (get-in (test-utils/read-test-state :game-map) [1 0 :contents :group-battleship-id]))))
  ...
  (context "dead submarine"
    (it "removes escort-id from carrier's group-submarine-ids"
      ...
      (let [dead {:type :submarine :owner :computer
        ...
        (combat/clear-escort-on-death dead))
      (should= [5] (get-in (test-utils/read-test-state :game-map) [0 0 :contents :group-submarine-ids]))
      ;; Non-matching carrier untouched
      (should= [3] (get-in (test-utils/read-test-state :game-map) [1 0 :contents :group-submarine-ids])))
    ...
    (it "leaves empty vector when last submarine dies"
      ...
      (let [dead {:type :submarine :owner :computer
                  :escort-carrier-id 10 :escort-id 3}]
        (combat/clear-escort-on-death dead))
      (should= [] (get-in (test-utils/read-test-state :game-map) [0 0 :contents :group-submarine-ids]))))
  ...
  (context "dead carrier"
    (it "releases escorts to seeking mode"
      ...
      (should= :seeking (get-in (test-utils/read-test-state :game-map) [0 0 :contents :escort-mode]))
      (should-be-nil (get-in (test-utils/read-test-state :game-map) [0 0 :contents :escort-carrier-id]))
      (should-be-nil (get-in (test-utils/read-test-state :game-map) [0 0 :contents :orbit-angle]))
      (should= :seeking (get-in (test-utils/read-test-state :game-map) [1 0 :contents :escort-mode]))
      (should-be-nil (get-in (test-utils/read-test-state :game-map) [1 0 :contents :escort-carrier-id]))
      ;; Non-matching escort untouched
      (should= :escorting (get-in (test-utils/read-test-state :game-map) [2 0 :contents :escort-mode]))
      (should= 99 (get-in (test-utils/read-test-state :game-map) [2 0 :contents :escort-carrier-id]))))
  ...
  (context "non-group unit"
    (it "does nothing for a unit without group fields"
      ...
      (let [dead {:type :army :owner :computer}]
        (combat/clear-escort-on-death dead))
      ;; Carrier untouched
      (should= 10 (get-in (test-utils/read-test-state :game-map) [0 0 :contents :carrier-id])))))
