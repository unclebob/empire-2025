...
(describe "set-coastline-follow-mode"
  ...
  (it "removes reason and target"
    (set-test-world! (build-test-map ["T"]))
    (set-test-unit (test-utils/game-map-atom) "T" :reason :some-reason :target [5 5])
    (set-coastline-follow-mode [0 0])
    (let [unit (get-in (test-utils/read-test-state :game-map) [0 0 :contents])]
...
(describe "move-coastline-unit"
  ...
  (it "moves transport along coastline"
    ...
    (should-be-nil (:contents (get-in (test-utils/read-test-state :game-map) [1 2]))))
  ...
  (it "does NOT wake for edge when started at edge"
    ...
    (let [{:keys [unit]} (get-test-unit (test-utils/game-map-atom) "T" :mode :coastline-follow)]
      (should= :coastline-follow (:mode unit))))
  ...
  (it "wakes up when steps exhausted"
    ...
    (set-test-player-map! (test-utils/read-test-state :game-map))
    (move-coastline-unit [1 2])
    (let [{:keys [unit]} (get-test-unit (test-utils/game-map-atom) "T" :mode :awake)]
      (should= :awake (:mode unit))
  ...
  (it "wakes up transport with armies when finding a bay"
    ...
    (let [unit (get-in (test-utils/read-test-state :game-map) [2 3 :contents])]
  ...
  (it "does NOT wake patrol-boat in bay (only transports wake for bays)"
    ...
    (set-test-world! (build-test-map ["#####"
                                             ...
                                             "##~##"
                                             "#####"]))
    (set-test-unit (test-utils/game-map-atom) "P"
                   :mode :coastline-follow
                   :army-count 2
                   :coastline-steps 50
                   :start-pos [2 2]
...
(describe "collision prevention"
  ...
  (it "prevents overwriting another unit when destination becomes occupied"
    ...
    (let [patrol-coords [1 2]
      ...
      (set-test-unit (test-utils/game-map-atom) "P" :mode :coastline-follow
      ...
      (should-not-be-nil (:contents (get-in (test-utils/read-test-state :game-map) [1 3])))  ;; Destroyer still there
      (let [patrol-unit (get-in (test-utils/read-test-state :game-map) [1 2 :contents])]
