...
(describe "in-bounds?"
  (it "returns true for coordinates within bounds"
    (should= true (#'empire.game-mechanics.visibility/in-bounds? 0 0 5 5))
    (should= true (#'empire.game-mechanics.visibility/in-bounds? 4 4 5 5))
    (should= true (#'empire.game-mechanics.visibility/in-bounds? 2 3 5 5)))

  (it "returns false for negative row"
    (should= false (#'empire.game-mechanics.visibility/in-bounds? -1 0 5 5)))

  (it "returns false for negative col"
    (should= false (#'empire.game-mechanics.visibility/in-bounds? 0 -1 5 5)))

  (it "returns false for row at height"
    (should= false (#'empire.game-mechanics.visibility/in-bounds? 5 0 5 5)))

  (it "returns false for col at width"
    (should= false (#'empire.game-mechanics.visibility/in-bounds? 0 5 5 5))))

(describe "should-stamp-country?"
  (it "returns truthy for computer army with country-id"
    (should (#'empire.game-mechanics.visibility/should-stamp-country?
              {:type :army :owner :computer :country-id 3})))

  (it "returns falsy for nil unit"
    (should-not (#'empire.game-mechanics.visibility/should-stamp-country? nil)))

  (it "returns falsy for player army"
    (should-not (#'empire.game-mechanics.visibility/should-stamp-country?
...
(describe "was-unexplored?"
  (it "returns true for nil cell in visible map"
    (let [visible-map [[nil nil] [nil nil]]]
      (should= true (#'empire.game-mechanics.visibility/was-unexplored? visible-map 0 0))))
  ...
  (it "returns false for revealed land cell"
    (let [visible-map [[{:type :land} nil] [nil nil]]]
      (should= false (#'empire.game-mechanics.visibility/was-unexplored? visible-map 0 0))))
