...
(describe "is-players?"
  (it "returns true for player city"
    (should (vis-core/is-players? {:type :city :city-status :player})))

  (it "returns true for cell with player unit"
    (should (vis-core/is-players? {:type :land :contents {:owner :player}})))

  (it "returns false for computer city"
    (should-not (vis-core/is-players? {:type :city :city-status :computer})))

  (it "returns false for cell with computer unit"
    (should-not (vis-core/is-players? {:type :land :contents {:owner :computer}})))

  (it "returns false for empty cell"
    (should-not (vis-core/is-players? {:type :land})))

  (it "returns false for free city"
    (should-not (vis-core/is-players? {:type :city :city-status :free}))))

(describe "is-computers?"
  (it "returns true for computer city"
    (should (vis-core/is-computers? {:type :city :city-status :computer})))

  (it "returns true for cell with computer unit"
    (should (vis-core/is-computers? {:type :land :contents {:owner :computer}})))

  (it "returns false for player city"
    (should-not (vis-core/is-computers? {:type :city :city-status :player})))

  (it "returns false for cell with player unit"
    (should-not (vis-core/is-computers? {:type :land :contents {:owner :player}})))

  (it "returns false for empty cell"
    (should-not (vis-core/is-computers? {:type :land}))))

(describe "cell-visibility-radius"
  (it "returns 1 for cell without unit"
    (should= 1 (vis-core/cell-visibility-radius {:type :land})))

  (it "returns 1 for cell with army"
    (should= 1 (vis-core/cell-visibility-radius
                  {:type :land :contents {:type :army}})))

  (it "returns 2 for cell with satellite"
    (should= 2 (vis-core/cell-visibility-radius
...
(describe "reveal-surrounding-cells!"
  (it "reveals cells within radius 1"
    (let [game-map [[{:type :land} {:type :sea} {:type :land}]
      ...
      (vis-core/reveal-surrounding-cells! result game-map 1 1 3 3 1)
  ...
  (it "clamps at map edges"
    (let [game-map [[{:type :land} {:type :sea}]
      ...
      (vis-core/reveal-surrounding-cells! result game-map 0 0 2 2 1)
      (let [final (mapv persistent! (persistent! result))]
        (should= {:type :land} (get-in final [0 0]))
        (should= {:type :sea} (get-in final [1 0]))
        (should= {:type :sea} (get-in final [0 1]))
