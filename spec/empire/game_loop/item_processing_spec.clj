...
;; ===== process-player-items-batch: loop mechanics (L219, L224, L230, L231) =====

(describe "process-player-items-batch loop"
  (before (reset-all-atoms!))

  (it "processes items counting from 0 (L219 0→1)"
    ;; With 100 items that all return :done, should process all 100
    ;; Mutant starting at 1 would only process 99
    (let [processed (atom 0)]
      ...
      (with-redefs [attention/item-needs-attention? (fn [_] false)
                    attention/set-attention-message (fn [_])]
        (ip/process-player-items-batch)
        ;; All 100 items should be processed (batch limit is 100)
        (should (empty? @atoms/player-items)))))

  (it "stops at batch limit of 100 (L224 >=→>)"
    ;; 150 items, should process exactly 100
    (let [n 5]
      (reset! atoms/game-map (make-land-map n))
  ...
  (it "increments processed counter on :done (L230 inc→dec)"
    ;; Need items that return :done but don't exhaust the list before counter hits 100.
    ;; Use a moving unit that returns nil from move-current-unit → process-auto-movement
    ;; returns :done and removes from list. But we cons new items to keep list non-empty.
    ;; Strategy: mock process flow so each item returns :continue (stays in list),
    ;; forcing the counter to reach 100.
    ;; A moving unit with steps remaining returns new-coords → :continue in process-auto-movement
    (let [call-count (atom 0)]
      (reset! atoms/game-map (vec (repeat 110 [(land-cell)])))
      ;; Unit at [0,0] in :moving mode with many steps
      ...
      (with-redefs [movement/move-unit
                    ...
                    attention/set-attention-message (fn [_])]
        (ip/process-player-items-batch)
        ;; Should stop at 100 iterations, not run forever
  ...
  (it "stops when paused (victory declared mid-batch)"
    ...
    (with-redefs [attention/item-needs-attention? (fn [_] false)
                  attention/set-attention-message (fn [_])]
      (ip/process-player-items-batch)
      ;; After processing first item, check-player-victory! declares victory
      ;; (no computer items), pauses game, stops batch
      (should @atoms/paused))))
