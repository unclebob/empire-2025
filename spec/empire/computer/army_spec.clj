...
(describe "update-backtrack trimming (L33)"
  (before (reset-all-atoms!))
...
(describe "attack-enemy deterministic combat (L115)"
  (before (reset-all-atoms!))
...
(describe "find-city-objective filters (L142, L143, L149)"
  (before (reset-all-atoms!))
...
(describe "move-toward-objective preferred-in-history fallback (L175)"
  (before (reset-all-atoms!))
...
(describe "explore-randomly history fallback (L212)"
  (before (reset-all-atoms!))
...
(describe "coast-walk multiple best candidates (L238)"
  (before (reset-all-atoms!))
...
(describe "adjacent-to-computer-city? (L252, L253)"
  (before (reset-all-atoms!))
...
(describe "find-nearest-unoccupied-coastal-cell (L259, L264, L266, L272)"
  (before (reset-all-atoms!))
...
(describe "find-nearest-cell-close-to-coast (L284, L290, L291)"
  (before (reset-all-atoms!))
...
(describe "fill-coastal-cell wake sentries (L325)"
  (before (reset-all-atoms!))
...
(describe "start-interior-exploration target calculation (L354)"
  (before (reset-all-atoms!))
...
(describe "find-and-execute-land-action city objective (L370)"
  (before (reset-all-atoms!))
...
(describe "process-random-explore goes sentry on coast after move (L391)"
  (before (reset-all-atoms!))
...
(describe "sentry army in city triggers fill-coastal-cell (L438)"
  (before (reset-all-atoms!))
...
(describe "move-toward-objective sovereignty passability (L176)"
  (before (reset-all-atoms!))
