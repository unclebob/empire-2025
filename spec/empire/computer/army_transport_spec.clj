...
(describe "army transport boarding"
  ...
  (it "boards an adjacent loading transport and updates visibility"
    (let [calls (atom [])]
      ...
      (with-redefs [empire.computer.shared.transport-query/find-adjacent-loading-transport (fn [pos unload-id]
                    ...
                    empire.computer.shared.action-resolution/board-transport (fn [from to] (swap! calls conj [:board from to]))
                    empire.computer.shared.movement/update-cell-visibility! (fn [pos owner] (swap! calls conj [:visibility pos owner]))
                    empire.computer.shared.transport-query/find-loading-transport (fn [& _] (swap! calls conj :fallback))
  ...
  (it "moves toward a distant loading transport when none are adjacent"
    ...
    (with-redefs [empire.computer.shared.transport-query/find-adjacent-loading-transport (constantly nil)
                  empire.computer.shared.transport-query/find-loading-transport (fn [unload-id]
                                                                                  (should= 5 unload-id)
                                                                                  [3 3])
