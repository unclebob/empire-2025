...
(describe "mutation: attack-enemy (L49, L52)"
  (before (reset-all-atoms!))
...
(describe "mutation: passable neighbors exclude computer units (L27)"
  (before (reset-all-atoms!))
...
(describe "mutation: no retreat when not threatened (L113)"
  (before (reset-all-atoms!))
...
(describe "mutation: in-bounds? boundary (L194)"
  (before (reset-all-atoms!))
...
(describe "mutation: detect-reflection-surface (L195, L203, L204, L206)"
  (before (reset-all-atoms!))

  ;; Map: 5 cols × 3 rows (from 3 strings of 5 chars)
  ;; max-c=4, max-r=2. Coords are [col, row].
...
(describe "mutation: escorting distance boundary (L450)"
  (before (reset-all-atoms!))
...
(describe "mutation: compute-distant-city-pairs boundary (L485)"
  (before (reset-all-atoms!))
...
(describe "mutation: find-unreserved-pair lazy init (L511)"
  (before (reset-all-atoms!))
...
(describe "mutation: find-position-between-cities (L524, L525, L534, L535)"
  (before (reset-all-atoms!))

  (it "midpoint first-coord is correct (L524)"
    ;; 61 cols × 1 row. Cities at [10,0] and [50,0]. Midpoint at [30,0].
  ...
  (it "midpoint second-coord is correct (L525)"
    ;; 1 col × 61 rows. Cities at [0,10] and [0,50]. Midpoint at [0,30].
...
(describe "mutation: find-refueling-sites (L547-L551)"
  (before (reset-all-atoms!))
...
(describe "mutation: pair-still-valid? (L625, L626)"
  (before (reset-all-atoms!))
...
(describe "mutation: carrier submarine slot cap (L689)"
  (before (reset-all-atoms!))
...
(describe "mutation: initial-orbit-angle (L698, L700)"
  (before (reset-all-atoms!))
...
(describe "mutation: orbit advances forward (L741)"
  (before (reset-all-atoms!))
...
(describe "mutation: legacy escort distance boundary (L902)"
  (before (reset-all-atoms!))
